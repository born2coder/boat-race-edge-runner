...
const predictionSchema = z.object({
  ...
  race_id: z.string(),
  model_version: z.enum(["poc-score-v0.1", "W_dynamic10_v1"]),
  strategy_version: z.enum(["fixed-3tickets-v0.1", "top3-flat100-v1"]),
  ...
  publication_mode: z.enum(["forward_observation_poc", "frozen_forward_hit_v1"]),
  official_performance_eligible: z.boolean(),
  publication_hash: sha,
}).superRefine((prediction, context) => {
  if (prediction.model_version !== "W_dynamic10_v1") return;
...
export const ingestPayloadSchema = z.object({
  ...
  decisions: z.array(z.object({
    race_id: z.string(),
    model_version: z.enum(["poc-score-v0.1", "W_dynamic10_v1"]).optional(),
