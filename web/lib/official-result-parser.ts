...
// Refunded races and dead heats still require archive reconciliation.
// A non-refunded abnormal finisher does not invalidate a verified winning three.
export function parseOfficialResult(html: string, raceId: string, roster: Array<{ lane_no: number; racer_id: string }>, observedAt: string): OfficialResult | null {
  ...
  const finishers = finishTable.find("tbody tr").toArray().map((row) => {
    ...
    const position = text(cells.eq(0).text());
    return {
      finish_position: /^[1-6]$/.test(position) ? Number(position) : null,
      result_code: /^[1-6]$/.test(position) ? null : position,
      lane_no: Number(text(cells.eq(1).text())), racer_id: text(cells.eq(2).find(".is-fs12").text()),
    };
  });
  const placed = finishers.filter((f) => f.finish_position !== null);
  if (finishers.length !== 6 || new Set(finishers.map((f) => f.lane_no)).size !== 6
    || new Set(placed.map((f) => f.finish_position)).size !== placed.length
    || finishers.some((f) => (f.finish_position === null && !["転", "落", "沈", "不", "失"].includes(f.result_code ?? ""))
  ...
  const sorted = [...finishers].sort((a, b) => (a.finish_position ?? 99) - (b.finish_position ?? 99));
  const payout = Number(payouts[0].replace(/[^\d]/g, ""));
  if (![1, 2, 3].every((position, i) => sorted[i]?.finish_position === position)
    || combination !== sorted.slice(0, 3).map((f) => f.lane_no).join("-") || !Number.isSafeInteger(payout) || payout <= 0) return null;
