...
export default async function EdgePage() {
  ...
  return <>
    ...
    <section className="edge-results" aria-labelledby="edge-results-title"><div className="section-heading split"><div><p className="section-kicker">TODAY / {dateLabel(new Date().toISOString())}</p><h2 id="edge-results-title">本日のEDGE候補</h2><p>全開催レースを対象に、期待値200%以上になった買い目だけを表示します。</p></div><span className="edge-count">候補 {open.length}件</span></div>{candidates.length === 0 ? <div className="edge-empty"><CircleHelp aria-hidden="true" /><div><h3>現在、表示できる候補はありません</h3><p>各レースの締切20分前前後に判定します。条件に届かなかったレースは表示されません。</p></div></div> : <div className="edge-candidate-grid">{candidates.map((c) => <article className={`edge-candidate ${c.status}`} key={c.edge_id}><header><div><strong>{c.venue_name} {c.race_no}R</strong><span>締切 {timeLabel(c.start_at)}</span></div><b>{c.status === "settled" ? (c.hit ? "的中" : "不的中") : c.expected_value_percent >= 300 ? "強いEDGE" : "EDGE候補"}</b></header><div className="edge-combination">{c.combination}</div><dl><div><dt>予測確率</dt><dd>{(c.predicted_probability * 100).toFixed(1)}%</dd></div><div><dt>オッズ</dt><dd>{c.odds_decimal.toFixed(1)}倍</dd></div><div><dt>期待値</dt><dd>{c.expected_value_percent.toFixed(0)}%</dd></div></dl>{c.status === "settled" && <p className="edge-settlement">結果 {c.result_combination ?? "—"} ／ 払戻 {c.payout_per_100_yen == null ? "—" : formatYen(c.payout_per_100_yen)}</p>}<small>取得 {new Date(c.observed_at).toLocaleString("ja-JP", { timeZone: "Asia/Tokyo" })}</small></article>)}</div>}</section>
